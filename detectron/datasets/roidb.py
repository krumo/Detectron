...
def combined_roidb_for_training(dataset_names, proposal_files, is_source=True):
    ...
    is_source_list = [is_source]*len(dataset_names)
    roidbs = [get_roidb(*args) for args in zip(dataset_names, proposal_files, is_source_list)]
    roidb = roidbs[0]
    for r in roidbs[1:]:
        roidb.extend(r)
    if is_source:
